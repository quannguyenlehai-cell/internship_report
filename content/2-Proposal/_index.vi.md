...
* Việc đối soát khuôn mặt hoàn toàn trên đám mây gây ra độ trễ cao và thất bại hoàn toàn khi mất kết nối internet.
...
- **Quản lý người dùng (User Management)**: Amazon Cognito (tích hợp qua AWS Amplify) xử lý xác thực quản trị viên an toàn, quản lý mã thông báo JWT và kiểm soát truy cập dựa trên vai trò (RBAC) kết hợp cùng AWS API Gateway.
...
- **Giai đoạn 1:** Phần cứng Edge & Thiết lập nòng cốt Đám mây (Tuần 1–4)
  + Nạp phần mềm (flash) cho ESP32-CAM AI-Thinker và lưu trữ đặc trưng khuôn mặt ban đầu vào bộ nhớ flash cục bộ.
...
- **Giai đoạn 2:** Giao diện Web & Tích hợp hệ thống (Tuần 5–7)
  + Cấu hình Amazon Cognito User Pools cho quyền truy cập và quản lý vai trò của quản trị viên.
  ...
  + Tiến hành kiểm thử ngắt kết nối mạng để xác minh tính năng truy cập ngoại tuyến 100% và tối ưu hóa hiệu suất phần cứng trong điều kiện ánh sáng yếu.
...
**Yêu cầu kỹ thuật**
- **Yêu cầu thành phần phần cứng**: ESP32-CAM (AI-Thinker), Khóa cửa điện từ hoặc khóa chốt Solenoid 5V/12V kèm Mô-đun Rơ-le Optocoupler 1 kênh, Mạch chuyển đổi FTDI USB-to-TTL (để nạp firmware), nguồn điện bên ngoài 5V/2A và công tắc cơ mở cửa khẩn cấp.
...
- Độ tin cậy ngoại tuyến 100%: Cửa hoạt động độc lập không phụ thuộc vào trạng thái mạng.
